...
const NAV_ITEMS = [
  {
    to: "/", label: "Dashboard", end: true, qaOnly: true,
  ...
  {
    to: "/protocols/new", label: "New Protocol", end: false, qaOnly: true,
  ...
  {
    to: "/drafts", label: "Drafts", end: false, qaOnly: true,
  ...
  {
    to: "/calibration", label: "Calibration", end: false, qaOnly: false,
  ...
  {
    to: "/disputes", label: "Disputes", end: false, qaOnly: false,
  ...
  {
    to: "/settings/users", label: "Settings", end: false, qaOnly: true,
  ...
  return (
    <div className="flex flex-col min-h-screen font-sans">
      ...
      <header className="sticky top-0 z-[100] flex items-center justify-between h-[var(--topbar-height)] px-6 bg-bg-primary border-b border-border-sec shadow-sm">
        <button onClick={() => navigate(isQA ? "/" : "/results")} className="flex items-center gap-2.5 p-0 bg-transparent border-none cursor-pointer">
          <LSGLogoMark size={30} />
      ...
      <div className="flex flex-1">
        <aside className="sticky top-[var(--topbar-height)] flex flex-col w-[var(--sidebar-width)] h-[calc(100vh-var(--topbar-height))] p-3 gap-1 bg-bg-primary border-r border-border-sec overflow-y-auto shrink-0">
          <p className="px-3 mb-1 text-[10px] font-semibold text-text-ter uppercase tracking-widest">Navigation</p>
          {visibleNav.map((item) => (
            <NavLink key={item.to} to={item.to} end={item.end}
              className={({ isActive }) =>
                `flex items-center gap-3 px-3 py-2 rounded-lg transition-all duration-200 ${
                  isActive ? "bg-bg-accent text-lsg-blue-dark font-semibold" : "text-text-sec hover:bg-bg-secondary"
                }`
              }
            >
