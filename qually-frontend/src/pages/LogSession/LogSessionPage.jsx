 ...
 * - Auditor field removed — the logged-in user is always the auditor.
 *   Their name is displayed as read-only in Session Details.
 * - Comments moved to after the question list, above the footer.
 * - Comments become mandatory when any question is answered No.
 *   The label updates to reflect this and the Submit button is blocked
 *   until comments are filled.
 * - Save Draft enabled as soon as one question is answered, even if
 *   session details are incomplete.
 * - Submit shows a specific warning listing exactly what is missing
 *   when the user clicks it before the session is complete.
 * - Subattribute panel only renders when the question actually has
 *   subattributes — "No sub-criteria defined" message removed.
 */
...
/**
 * Only renders when the question has subattributes.
 * Silently returns null for questions with no sub-criteria.
 */
        ...
        <div key={sub.subattributeId} className="pl-4 border-l-2 border-border-ter">
          ...
          <div className="flex flex-wrap gap-2">
            {sub.subattributeOptions?.map((opt) => {
              const isSelected =
                opt.subattributeOptionId === selectedOptions[sub.subattributeId];
              return (
                <button
...
export default function LogSessionPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      // Exclude the logged-in user from auditable members
  ...
  } = useLogSession(protocol, user?.userId);

  // Track whether user has attempted to submit (to show missing fields warning)
  ...
  const memberOptions = auditableUsers.map((u) => ({
    value: u.userId,
    ...
    <div className="max-w-[860px] mx-auto px-8 py-10">
      ...
      <section className="bg-bg-primary border border-border-sec rounded-xl p-6 mb-6 shadow-card">
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
          ...
          {/* Auditor — read-only, sourced from auth context */}
          <div>
            <label className="block text-xs font-bold text-text-sec uppercase tracking-wider mb-1.5">
              Auditor
            </label>
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-2">
          <span className="text-[10px] font-bold text-text-ter uppercase tracking-widest">
            Questions
          </span>
            ...
            return (
              ...
              >
                <div className="p-6">
                  ...
                  {/* Subattribute panel — only when question has subattributes AND answer is NO */}
      ...
      <section className="bg-bg-primary border border-border-sec rounded-xl p-6 mb-6 shadow-card">
        ...
        {commentsRequired && (
          <p className="text-xs text-text-ter mb-2">
            Required because one or more questions were answered No.
          </p>
        )}
        <textarea
          value={meta.comments}
      ...
      {totalCount > 0 && (
        <footer className="sticky bottom-6 bg-bg-primary/90 backdrop-blur-md p-4 border border-border-ter rounded-2xl shadow-lg flex items-center justify-between gap-4">
          <div className="min-w-0 flex-1">
            {/* Missing fields warning — only shown after a submit attempt */}
          ...
          <div className="flex gap-2 shrink-0">
            ...
            <button
              ...
              disabled={saving}
              className={[
                "px-5 py-2 text-sm font-bold text-white rounded-md transition-colors disabled:opacity-40 disabled:cursor-not-allowed",
                canSubmit
                  ? "bg-lsg-blue hover:bg-lsg-blue-dark"
                  : "bg-lsg-blue/60 hover:bg-lsg-blue/70",
              ].join(" ")}
            >
